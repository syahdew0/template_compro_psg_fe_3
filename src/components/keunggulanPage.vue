<template>
  <section id="keunggulan" class="min-h-screen flex items-center bg-white p-5 pt-10">
    <div class="relative z-10 flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-2 p-6 bg-white backdrop-blur-lg mx-auto w-full max-w-7xl">
      <!-- Mockup Images -->
    ...
    <div class="w-full md:w-1/2 text-center md:text-left space-y-6">
      ...
      <div class="space-y-3">
        ...
        <div
          ...
          :key="index"
          class="flex items-center p-3 bg-white border-2 border-gray-100 rounded-lg cursor-pointer transform hover:scale-105 hover:bg-gray-100 transition-all duration-300"
        >
      ...
      <!-- Tombol -->
      <button
        class="mt-4 px-6 py-3 bg-[#00B1D6] text-white border-2 border-[#00B1D6] font-normal rounded-full shadow-md hover:bg-white hover:text-[#00B1D6] transition-all duration-300 flex items-center mx-auto md:mx-0"
      >
...
<!-- KENAPA MEMILIH KAMI --> 
<div class="relative w-full max-w-7xl bg-white p-10 flex items-center">
      <!-- Fitur List -->
      <div class="relative space-y-5 w-1/2 ">
        <div 
          v-for="(feature, index) in rentalFeatures" 
          :key="feature.title"
          class="flex items-center space-x-4 relative">
          <!-- Garis konektor -->
          <div v-if="index !== 0" class="absolute w-20 h-0.5 bg-[#00B1D6] right-[-80px] top-1/2"></div>
          ...
          <div 
            ...
            <h3 class="font-normal text-lg" :class="feature.isBlue ? 'text-white' : 'text-[#00B1D6]'">
              {{ feature.title }}
            </h3>
    ...
    <!-- Kotak biru panjang -->
  <div class="bg-[#00B1D6] text-white px-5 py-10 w-full max-w-[550px] absolute right-0 flex justify-end items-center ml-auto">
    <h2 class="text-3xl font-normal">Kenapa memilih kami?</h2>
   ...
   <div class="absolute left-10 transform -translate-x-12 grid grid-cols-2 grid-rows-2 gap-3 z-10">
