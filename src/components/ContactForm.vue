...
onMounted(() => {
  ...
  try {
    ...
    const badgeData = parse(data.contact_form_badge)
    if (badgeData) badge.value = badgeData.title || ''

    const titleData = parse(data.contact_form_title)
    if (titleData) {
    ...
    const buttonData = parse(data.contact_form_button)
    if (buttonData) buttonText.value = buttonData.title || 'Send Message'

    const fieldsData = parse(data.contact_form_fields)
    ...
    console.error(' Gagal parsing contact form:', err)
  }
})
